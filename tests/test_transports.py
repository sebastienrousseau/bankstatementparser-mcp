...
from bankstatementparser_mcp import _cli, _transports, server
...
def test_version_flag(capsys: pytest.CaptureFixture[str]) -> None:
    with pytest.raises(SystemExit) as info:
        _cli.serve("srv", ["--version"], "bankstatementparser-mcp", "9.9.9")
    assert info.value.code == 0
    assert "bankstatementparser-mcp 9.9.9" in capsys.readouterr().out


def test_main_defaults_to_stdio(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[Any] = []
    monkeypatch.setattr(
        server.mcp, "run", lambda *a, **k: calls.append((a, k))
    )
    server.main([])
    assert calls == [((), {})]
