...
import bankstatementparser_mcp.adapters as adapters  # noqa: E402
import bankstatementparser_mcp.server as srv  # noqa: E402
...
def _gateway_tool_map() -> dict:
    ...
    return {t.name: t for t in adapters._gateway_tools()}
...
def test_as_langchain_tools_missing_extra_raises(monkeypatch):
    ...
    try:
        ...
        raised = True
        assert "bankstatementparser-mcp[langchain]" in str(exc)
    assert raised is True
...
def test_as_crewai_tools_missing_extra_raises(monkeypatch):
    ...
    try:
        ...
        raised = True
        assert "bankstatementparser-mcp[crewai]" in str(exc)
    assert raised is True
...
def test_as_llamaindex_tools_missing_extra_raises(monkeypatch):
    ...
    try:
        ...
        raised = True
        assert "bankstatementparser-mcp[llamaindex]" in str(exc)
    assert raised is True


# ---------------------------------------------------------------------------
# The adapters are plain module functions, not @mcp.tool: the server's
