# Contributing to bankstatementparser-mcp

Thank you for your interest in contributing to bankstatementparser-mcp. This guide covers
the development workflow and standards.

`bankstatementparser-mcp` is the Model Context Protocol (MCP) server of the **bankstatementparser
suite** - alongside the core [`bankstatementparser`](https://github.com/sebastienrousseau/bankstatementparser)
library and the [`bankstatementparser-lsp`](https://github.com/sebastienrousseau/bankstatementparser-lsp)
Language Server. It depends on `bankstatementparser` and exposes its public API as agent
...
```bash
# Clone and install
git clone git@github.com:sebastienrousseau/bankstatementparser-mcp.git
...
The package depends on the core `bankstatementparser` library and `mcp`; both are
...
5. **Run linters**:
   ```bash
   poetry run ruff check bankstatementparser_mcp/
   poetry run mypy bankstatementparser_mcp/
   poetry run black --check bankstatementparser_mcp/ tests/
   ```
