...
# Releasing bankstatementparser-mcp
...
bankstatementparser-mcp tracks
[`bankstatementparser`](https://github.com/sebastienrousseau/bankstatementparser) version-for-
version: when bankstatementparser ships `0.0.X`, bankstatementparser-mcp ships a matching
...
for MCP-only bug fixes that don't require a bankstatementparser bump.
...
3. Push a signed tag:
   ...
   ```bash
   git tag -s vX.Y.Z -m "bankstatementparser-mcp vX.Y.Z" <merge-commit>
...
- Confirm the version is live on
  [PyPI](https://pypi.org/project/bankstatementparser-mcp/) and the GitHub release is
  published (not draft).
- Verify a clean install: `pip install bankstatementparser-mcp==X.Y.Z`.
- The `docker.yml` workflow publishes a matching multi-arch image to
  `ghcr.io/sebastienrousseau/bankstatementparser-mcp:X.Y.Z` (and refreshes
  `:latest` when the release lands on `main`). The workflow smoke-tests
  the pulled image by running the `bankstatementparser_mcp` import check, so a
...
- **PyPI trusted publisher** (`release.yml`): configured at
  ...
  set is `repo:sebastienrousseau/bankstatementparser-mcp:environment:pypi` with
...
- **GHCR multi-arch image** (`docker.yml`): published with the
  ...
  image: `docker run -i --rm ghcr.io/sebastienrousseau/bankstatementparser-mcp:X.Y.Z`.
